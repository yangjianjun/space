...
function _error(_this, show_id, show_text) {
	...
	_this.css("background-color","#00b9f5")
	$("#" + show_id).html('<span style="color:#00b9f5;">(×)'+show_text+'</span>');
}
//比较数字大小或比较字符串长短（内部调用）
...
function _core (my_config) {
	...
	switch (_config.mod) {
		case 'match':
			if (_config.arg == 'email' || _config.arg == 'phone' || _config.arg == 'qq' || _config.arg == 'idcard' || _config.arg == 'zh' || _config.arg == 'tname' || _config.arg == 'com') {
