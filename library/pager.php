...
//		$subPageCss2Str.=$this->_lang['current_page'] . $this->current_page."/" . $this->pageNums." ";
		...
		if($this->current_page > 1){
			$firstPageUrl=$this->subPage_link."1";
			$prewPageUrl=$this->subPage_link.($this->current_page-1);
			$subPageCss2Str.="<td><a href='$firstPageUrl'>{$this->_lang['index_page']}</a></td>";
			$subPageCss2Str.="<td><a href='$prewPageUrl'>{$this->_lang['pre_page']}</a></td>";
		}else {
			$firstPageUrl=$this->subPage_link."1";
			$subPageCss2Str.="<td><a href='$firstPageUrl'>{$this->_lang['index_page']}</a></td>";
//			$subPageCss2Str.="<td>{$this->_lang['pre_page']} </td>";
		}
		...
		for($i=0;$i<count($a);$i++){
			$s=$a[$i];
			if($s == $this->current_page ){
			 	$subPageCss2Str.="<td><span>".$s."</span></td>";
			}else{
//				 echo "*";
				 $url=$this->subPage_link.$s;
				 $subPageCss2Str.="<td><a href='$url'>".$s."</a></td>";
			}
		}
		...
		if($this->current_page < $this->pageNums){
			$lastPageUrl=$this->subPage_link.$this->pageNums;
			$nextPageUrl=$this->subPage_link.($this->current_page+1);
			$subPageCss2Str.="<td><a href='$nextPageUrl'>{$this->_lang['next_page']}</a></td> ";
			$subPageCss2Str.="<td><a href='$lastPageUrl'>{$this->_lang['last_page']}</a></td> ";
		}else {
			$lastPageUrl=$this->subPage_link.$this->pageNums;
//			$subPageCss2Str.="<td>{$this->_lang['next_page']} </td>";
			$subPageCss2Str.="<td><a href='$lastPageUrl'>{$this->_lang['last_page']}</a> </td>";
		}
		
		$subPageCss2Str = "<table border='0' cellpadding='0' cellspacing='5'><tbody><tr>".
        $subPageCss2Str."</tr></tbody></table>";
